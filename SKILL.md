...
name: smart-memory
version: 2.3.3
description: 分层长期记忆管理系统：任务前自动 TF-IDF 召回相关记忆，任务后持久化知识卡片，支持语义去重、时间衰减权重、全量索引重建、LLM 结构化收割、聚类合成、跨卡关联、信号分析与成熟度报告，可从 self-learning-skills 一键迁移历史数据。
---
...
| `recalc-importance` | 基于信号数据批量重算所有卡片的 importance |
...
| `synthesize --threshold 0.55 --min-cluster-size 3 --auto-write` | 聚类相似卡片生成综述知识簇 |
| `cross-link --threshold 0.35 --top-k 5 --auto-write` | 向量相似度发现跨卡隐藏关联，写入 graph.json |
| `signal-analysis --days 30 --auto-write` | 分析信号数据更新成熟度评分 |
| `maturity --limit 50` | 知识成熟度报告，A~D 四级分布 |
